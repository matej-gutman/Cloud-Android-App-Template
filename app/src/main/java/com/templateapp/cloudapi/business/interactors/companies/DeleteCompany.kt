...
import com.templateapp.cloudapi.business.domain.util.SuccessHandling.Companion.SUCCESS_COMPANY_DELETED
...
class DeleteCompany(
    ...
    fun execute(
        ...
        val response = service.deleteCompany(
            authorization = authToken.token,
            id = id
        )

        if(response.response != "201"){ // failure
            throw Exception(ErrorHandling.ERROR_DELETING_COMPANY)
        }else{
            // delete from cache
            cache.deleteCompany(id)
            // Tell the UI it was successful
            emit(DataState.data<Response>(
                data = Response(
                    message = SUCCESS_COMPANY_DELETED,
